      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 40, marginBottom: 48 }}>
        ...
        <div>
          ...
          {[{ to: '/', label: 'Home' }, { to: '/learning-hub', label: 'Learning Hub' }, { to: '/auth?mode=register', label: 'Register' }, { to: '/about', label: 'About Us' }].map(({ to, label }) => (
            <div key={to} style={{ marginBottom: 10 }}><Link to={to} style={{ color: 'var(--text-secondary)', fontSize: 14, textDecoration: 'none', transition: 'color 0.2s' }} onMouseEnter={e => e.target.style.color = 'var(--accent-green)'} onMouseLeave={e => e.target.style.color = 'var(--text-secondary)'}>{label}</Link></div>
          ))}
        </div>
        <div>
          ...
          {['DevOps Engineering', 'Site Reliability Engineering', 'Python for Industry', 'React.js Development'].map(c => (
            <div key={c} style={{ marginBottom: 10 }}><Link to="/learning-hub" style={{ color: 'var(--text-secondary)', fontSize: 14, textDecoration: 'none' }}>{c}</Link></div>
          ))}
        </div>
        <div>
          ...
          <p style={{ color: 'var(--accent-green)', fontSize: 13, fontFamily: 'var(--font-mono)', marginTop: 16 }}>{/* comment */} Mantra × Systems = Mantrixa</p>
        </div>
      </div>
      <div style={{ borderTop: '1px solid var(--border)', paddingTop: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>© 2024 Mantrixa. Crafted with ❤️ by Bhuvan & Tarun.</p>
        <p style={{ color: 'var(--text-muted)', fontSize: 13, fontFamily: 'var(--font-mono)' }}>v1.0.0</p>
      </div>
    </div>
  </footer>
);
